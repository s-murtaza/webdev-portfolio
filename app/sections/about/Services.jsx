...
export function TimeLine() {
	...
	return (
		<LazyMotion features={domAnimation}>
			<div className="my-6">
				<HeadingDivider title="Services"></HeadingDivider>
			</div>
			<ul
				ref={carouselRef}
				onScroll={handleScroll}
				className="flex flex-row flex-nowrap gap-3 mt-14 justify-between overflow-x-auto snap-x cursor-pointer hide-scroll-bar"
			>
				<>
								...
								<ul className="list-disc ml-4">
									{item.text.map((text, index)=>(
										<li className="my-1" key={index}>
											<p className="tracking-wide " tabIndex="0">{text}</p>
										</li>
									))}
								</ul>
							</li>
