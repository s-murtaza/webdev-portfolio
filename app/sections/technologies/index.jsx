		...
		<LazyMotion features={domAnimation}>
			<section id="tech" className="section">
				...
				<p
					ref={textRef}
					tabIndex="0"
					className="my-5 text-2xl mx-auto"
					style={{
						...
						{TECHNOLOGIES.map((tech, index) => {
							return (
								...
								>
									...
									<span>{tech.items[0].icon}</span>
									<div className="flex w-64 items-center flex-wrap gap-x-5 gap-y-8">
										{tech.items.map((item) => (
											<div key={item.name} className="group relative flex">
												...
												<span
													className="group-hover:opacity-100 group-focus-within:opacity-100 transition-opacity bg-gray-800 text-sm text-gray-100 rounded-md absolute
    -translate-x-1/2 translate-y-full opacity-0 mt-2 mx-auto px-2 w-max"
												>
													{item.name}
												</span>
											</div>
